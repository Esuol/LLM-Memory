...
#### 1. SSE 流式进度推送（最高优先级，UX 改善最大）✅ 已完成
...
#### 2. `embedDocuments` 替换多次 `embedQuery`（性能提升）✅ 已完成
...
#### 3. 边 Embedding 边 Upsert（流水线，减少等待）✅ 已完成
...
`fetchFilesInBatches` 接受 `onProgress` 回调，每批完成后推送进度，用户可看到"拉取第 X/N 批"。

#### 6. 失败诊断信息 ✅ 已完成

- 文件拉取失败数实时上报：`📄 成功拉取 72/89 个文件（⚠️ 17 个失败，可能触发限流）`
- Embedding 批次失败上报：`⚠️ 第 3/7 批 Embedding 失败，已跳过`
